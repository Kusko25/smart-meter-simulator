...
csv_path = base_path / "../../fake_data_results_alt.csv"
csv_path_fp = base_path / "../../fake_data_results_alt_fp.csv"
fig_path_error = base_path / "../../fake_data_results_alt_error.png"
fig_path_fp = base_path / "../../fake_data_results_alt_fp_var.png"
...
def gather_data():
    ...
    threshold_data = pd.read_csv(base_path / "data/threshold_data.csv", index_col="LCLid")
    ...
    N_SMART_METERS = 200
    for threshold_id in ["threshold_soft", "threshold_hard"]:
        for _ in range(5): # Repeat 5 times to average over randomness
            for j in range(0,20,1):

                run_data = input_data.sample(N_SMART_METERS + 2)
                malicious = [True]*(int(j+1)) + [False]*(N_SMART_METERS - int(j+1))
                shuffle(malicious)
                ...
                for i in range(N_SMART_METERS):
                    ...
                    id = run_data.iloc[i]["LCLid"]
                    threshold = threshold_data.loc[id, threshold_id]
    ...
    out_df = out_df.groupby(["n_malicious", "threshold_method"]).mean().reset_index()
...
def plot_data_fp():
    ...
    data["Threshold Method"] = data["threshold_method"].apply(lambda x: "Strict" if x == "threshold_hard" else "Permissive")
    data["False Positives"] = data["false_positives"] / data["n_smart_meters"]
    sns.boxplot(data=data, x="False Positives", hue="Threshold Method", ax=ax1, hue_order=["Strict", "Permissive"])
...
def plot_data():
    ...
    ax1.set_ylabel("False Positives (Average)")
    plt.savefig(fig_path_fp)
    ...
    ax1.set_xlabel("Number of Malicious Smart Meters (Total: 200)")
    ax1.set_ylabel("Relative Error (%) (Average)")
